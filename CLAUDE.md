...
3. **`netlify/functions/poll-status.mjs` + `netlify/functions/stats.mjs` + `netlify/functions/record-click.mjs`** — connectivity history and click counting. See "Downtime statistics" below.
...
**Do not reintroduce ESP32Servo.** It is broken on this board *and fails silently*, which is why the servo appeared dead while every ack said success. The `nano_nora` variant's RGB boot animation drives the LED with `analogWrite()`, leaving LEDC timers at 1000 Hz that it never releases; ESP32Servo keeps separate private bookkeeping, cannot see them, and hands the servo a 1000 Hz channel. A servo needs a 20 ms frame, so it neither moves nor buzzes. Worse, ESP32Servo discards `ledcAttachChannel()`'s return and `Servo::attached()` only reports its own flag, so the failure is invisible. The firmware now frees the RGB LED's LEDC channels at boot and calls `ledcAttach(D6, 50, 14)`, **checking the return**. Measured: 1000 Hz via the library, 50 Hz via raw LEDC. 14-bit is pinned because 16-bit at 50 Hz is rejected outright by the ESP32-S3.
...
The 24 h and 7 d rows lead with the number of trigger requests: `24h · 3 clicks · 0 drops · 0s down`. Clicks are recorded by **`record-click.mjs`**, which the PWA calls **fire-and-forget alongside** the `/trigger` POST — never awaited, errors swallowed, so a fault in telemetry can never stop the door opening.

It is a separate function rather than a few lines inside `trigger.js` for two reasons: `trigger.js` is a runtime-API-**v1** CommonJS function and v1 functions do not get the Blobs context (`getStore()` throws), and it is the garage-critical path. The trade-off is that this counts clicks *the PWA made*, not every request that reached `/trigger` — a `curl` straight to `/trigger` opens the door uncounted. That is the right side to err on.

Timestamps live under their own `clicks` blob key, deliberately **not** inside `state`: `poll-status.mjs` rewrites the whole `state` blob every minute, so sharing a key would make every click race a poll. Writes are read-modify-write, so two clicks in the same instant can lose one — acceptable given the 10 s cooldown, and undercounting by one beats blocking a door open.
